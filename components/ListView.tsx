             ...
             >
                <div className="col-span-4 md:col-span-5">
                   <h3 className="text-xl md:text-3xl font-light text-slate-800 group-hover:translate-x-2 transition-transform duration-300 flex items-center gap-2">
                     {item.title}
                     <ArrowUpRight size={16} className="opacity-0 group-hover:opacity-100 -translate-y-1 translate-x-1 transition-all text-slate-400" />
                   </h3>
                </div>
                ...
                <div className="col-span-4 md:col-span-2 text-right">
                   <span className={`inline-block px-3 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase border ${
                     item.type === SectionType.PROJECT ? 'border-blue-200 text-blue-600 bg-blue-50' : 
                     item.type === SectionType.EXPERIENCE ? 'border-purple-200 text-purple-600 bg-purple-50' : 'border-slate-200 text-slate-500'
                   }`}>
                     {item.type}
                   </span>
                </div>
