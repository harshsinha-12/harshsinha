        ...
        <div className="container mx-auto px-6 py-32 md:py-40 min-h-screen flex flex-col md:flex-row gap-16">
                ...
                <div className="col-span-2 md:col-span-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {item.techStack.map(t => (
                      <span key={t} className="font-semibold text-slate-800">{t}</span>
                    ))}
                  </div>
                </div>
              )}
          ...
          <div className="w-full md:w-1/3 flex flex-col gap-4">
             ...
             <div className="w-full aspect-[4/5] bg-slate-200 rounded-2xl overflow-hidden relative shadow-inner">
                <div className={`absolute inset-0 opacity-30 ${item.colorTheme.replace('bg-', 'bg-gradient-to-tr from-transparent to-')}`} />
                <div className="absolute bottom-8 left-8">
                   <h4 className="text-6xl font-bold text-white opacity-50 mix-blend-overlay">{item.title.substring(0, 2)}</h4>
                </div>
             </div>
          </div>
